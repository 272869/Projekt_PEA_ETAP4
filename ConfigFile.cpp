...
void ConfigFile::parseConfig() {
    ...
    if (mode == "generate")
        ...
        handleTestMode(config["fileName"], std::stod(config["probability"]), std::stoi(config["populationSize"]),std::stoi(config["populationCopyNumber"]),std::stoi(config["generationNumber"]),std::stoi(config["selectionType"]),std::stoi(config["crossoverType"]) );
}
